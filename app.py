...
# E-maily pro urgence (ZDE SI PŘEPIŠTE NA REÁLNÉ E-MAILY KOLEGŮ)
EMAIL_IWONSKI = "jiri.iwonski@firma.cz"
EMAIL_CEJKA = "martin.cejka@firma.cz"
            ...
            with st.container(border=True):
                ...
                if zaznam.empty:
                    ...
                    if je_po_termínu:
                        ...
                        c1.caption(f":red[Chybí částka! Termín byl 25.{deadline_m}.]")
                    else:
                    ...
                    if row['Iwonski'] != "":
                        ...
                        if role == "Jiří Iwonski":
                            ...
                            if dny_cekani >= 5:
                                ...
                                telo = urllib.parse.quote(f"Ahoj Jiří,\n\nprosím tě o schválení částky {castka_format} pro {agregator} v aplikaci.\n\nDíky, Martin")
                        ...
                        if role == "Martin Čejka":
                            ...
                            if row['Iwonski'] != "":
                                ...
                                if dny_cekani >= 5:
                                    ...
                                    predmet = urllib.parse.quote(f"Urgence schválení: {agregator} za {vybrany_mesic}")
                                    telo = urllib.parse.quote(f"Ahoj Martine,\n\nprosím tě o finální schválení částky {castka_format} pro {agregator} v aplikaci.\n\nDíky, Martin")
...
with tabs[-1]:
    ...
    if df.empty:
        ...
        st.dataframe(df_zobrazeni, use_container_width=True, hide_index=True)df = load_data()
...
with tabs[-1]:
    ...
    if df.empty:
        ...
        st.dataframe(df_zobrazeni, use_container_width=True, hide_index=True)st.sidebar.title("⚙️ Nastavení")
role = st.sidebar.selectbox("Přihlášen jako:", ["Martin Urban", "Jiří Iwonski", "Martin Čejka"])
vybrany_mesic = st.sidebar.selectbox("Fakturační měsíc:", mesice)
